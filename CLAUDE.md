...
python -m pip install -e . --no-build-isolation   # resolves aindy-runtime>=2.13.0,<3.0 from PyPI
...
```toml
aindy-runtime>=2.13.0,<3.0    # pyproject.toml — the COMPATIBILITY range
```
```
aindy-runtime==2.13.0         # constraints.txt — the BUILD pin
```
...
CI installs the published runtime from PyPI (the pinned `aindy-runtime>=2.13.0,<3.0`
