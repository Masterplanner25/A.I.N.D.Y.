...
> **Ownership & status (apps-monolith).** Brought over from the pre-split monolith during
> DOCS-MIGRATION-2 as the app-owned evolution roadmap. Phases 1–4 are runtime/platform hardening
> (complete) owned upstream by `aindy-runtime`; they are retained here as historical context for
> where current work sits. Phase 5 (Agentics + Nodus convergence) is the current phase and spans both
...
> `docs/operations/INVARIANTS.md`, while runtime invariants are owned by `aindy-runtime`.
...
- Preserve all invariants in `docs/operations/INVARIANTS.md`.
...
- Infinity Algorithm Support System evolution is defined in `docs/infinity/INFINITY_ALGORITHM_SUPPORT_SYSTEM.md`.
- RippleTrace evolution is defined in `docs/domains/RIPPLETRACE.md`.
- Search System evolution is defined in `docs/domains/SEARCH_SYSTEM.md`.
- Freelancing System evolution is defined in `docs/domains/FREELANCING_SYSTEM.md`.
- Social Layer evolution is defined in `docs/domains/SOCIAL_LAYER.md`.
- ARM evolution is defined in `docs/domains/AUTONOMOUS_REASONING_MODULE.md`.
- Masterplan SaaS evolution is defined in `docs/domains/MASTERPLAN_SAAS.md`.
- Implementation docs utility audit is defined in `docs/verification/IMPLEMENTATION_DOCS_AUDIT.md`.
- Agentics reality, corrected A.I.N.D.Y. + Nodus architecture, and completion roadmap are defined in `docs/architecture/AGENTICS.md`.
...
Focus areas:
- Improve test coverage for critical routes and services (`docs/operations/TESTING_STRATEGY.md`).
...
- Enforce migration discipline and schema validation (`docs/operations/INVARIANTS.md` checklist).
...
- Invariants in `docs/operations/INVARIANTS.md` have test coverage per `docs/operations/TESTING_STRATEGY.md`.
...
- Deployment checks block startup on schema drift (aligned with `docs/operations/INVARIANTS.md`).
...
- ✅ Horizontal deployment does not violate invariants or session isolation (per `docs/operations/INVARIANTS.md`).
...
> detailed in `docs/architecture/AGENTICS.md` and `docs/domains/AUTONOMOUS_REASONING_MODULE.md`.
...
Execution checklist:
1. `tools/maintenance/backfill_user_ids.py` added and executed (dry-run; apply when needed).
...
* Documentation for the Autonomous Intelligence layer is added to `docs/domains/RIPPLETRACE.md` (Autonomous Execution Layer section).
...
2. Multi-tenant isolation is enforced at the DB level and documented in `docs/domains/MASTERPLAN_SAAS.md`.
...
* Documentation in `docs/domains/RIPPLETRACE.md` and relevant architecture docs references the Advanced Intelligence roadmap.

## 7. Change Governance Rules
- No evolution phase may violate `docs/operations/INVARIANTS.md`.
...
- Invariants remain intact and tested (`docs/operations/INVARIANTS.md`).
...
- API contract updates reflected in `docs/api/API_CONTRACTS.md`.
...
- Tests executed per `docs/operations/TESTING_STRATEGY.md` (record command outputs or summaries).
- Alembic revision matches head (`alembic current` equals `alembic heads`).
- Schema vs. migration checks completed (`docs/operations/INVARIANTS.md` checklist).
