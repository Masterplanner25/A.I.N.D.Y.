...
> corrected in `docs/architecture/AGENTICS.md` and `docs/domains/AUTONOMOUS_REASONING_MODULE.md`
...
- `apps/analytics/services/scoring/infinity_service.py`
...
- `apps/automation/infinity_loop.py`
...
- watcher signal ingestion and KPI reads in `apps/analytics/services/scoring/infinity_service.py`
- daily score recalc in `AINDY/platform_layer/scheduler_service.py`
...
- legacy calculation endpoints in `apps/analytics/routes/main_router.py`
- standalone formulas in `apps/analytics/services/calculations/calculation_services.py`
...
- KPI scoring pipeline in `apps/analytics/services/scoring/infinity_service.py`
...
- `AINDY/agents/agent_runtime.py` injects Infinity KPI context into plan generation
- `AINDY/agents/agent_runtime.py` now also injects categorized memory context before execution
- completed agent runs trigger `apps/analytics/services/orchestration/infinity_orchestrator.execute()`
...
- `_decide()` in `apps/analytics/services/orchestration/infinity_loop.py` performs threshold-based adjustment logic
...
- `AINDY/memory/memory_scoring_service.py` now ranks relevant memories into failure/success/pattern signals
- task completion and major flows write to memory via `AINDY/memory/memory_capture_engine.py`
...
- standalone calculation endpoints in `apps/analytics/routes/main_router.py` no longer define the canonical control path; the remaining bypass is deeper domain execution that still runs before Infinity
- many domain services that mutate state without any Infinity orchestration boundary
- flow execution in `AINDY/runtime/flow_engine/runner.py`
- agent execution initiation in `AINDY/agents/agent_runtime.py`
...
- `apps/analytics/services/orchestration/infinity_orchestrator.py`
- `apps/analytics/services/orchestration/infinity_loop.py`
- `apps/masterplan/routes/score_router.py`
...
- `apps/analytics/services/orchestration/infinity_orchestrator.py`
- `apps/identity/services/identity_boot_service.py`
- `AINDY/memory/memory_capture_engine.py`
...
- `apps/analytics/services/orchestration/infinity_loop.py`
- `apps/analytics/services/orchestration/infinity_orchestrator.py`
- future reasoning files from `docs/domains/AUTONOMOUS_REASONING_MODULE.md`
...
- `apps/analytics/services/scoring/infinity_service.py`
- `apps/analytics/services/orchestration/infinity_orchestrator.py`
- `apps/masterplan/services/eta_service.py`
- `apps/masterplan/services/projection_service.py`
- `apps/analytics/user_score.py`
- `apps/automation/infinity_loop.py`
...
This document aligns with `docs/architecture/AGENTICS.md`:
...
This document aligns with `docs/domains/AUTONOMOUS_REASONING_MODULE.md`:
...
This document depends on `docs/infinity/INFINITY_ALGORITHM_SUPPORT_SYSTEM.md`:
...
This document aligns with `docs/architecture/EVOLUTION_PLAN.md`:
