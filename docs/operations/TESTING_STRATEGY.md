...
- In the extracted apps repo, those helpers assume installed `aindy-runtime`
  rather than a sibling `AINDY/` checkout.
...
- The runtime dependency boundary is documented in
...
- Tests must validate all invariants defined in `docs/operations/INVARIANTS.md`.
- Any change affecting invariants requires updated tests.

### B. Service-Level Unit Tests
- All business logic in `AINDY/core/`, `AINDY/runtime/`, `AINDY/platform_layer/`, `AINDY/agents/`, and `apps/ or AINDY/platform_layer/` must have unit tests.
...
- API contracts must match `docs/api/API_CONTRACTS.md`.
...
### A. OpenAI (`apps/masterplan/services/genesis_ai.py`)
...
### B. DeepSeek Integration (`apps/arm/services/deepseek/deepseek_code_analyzer.py`)
