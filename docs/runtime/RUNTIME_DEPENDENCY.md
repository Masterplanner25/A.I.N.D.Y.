...
- installed runtime version: `2.15.0` (checked by **import and path** —
  ...
  `RUNTIME_2_12_0_UPGRADE.md` §1; re-checked clean on 2.13.0, 2.14.0 and 2.15.0, `RUNTIME_2_15_0_UPGRADE.md` §1.
...
- apps repo dependency: range `aindy-runtime>=2.15.0,<3.0` in `pyproject.toml`, build pin
  `aindy-runtime==2.15.0` in `constraints.txt`, asserted equal by
  `tests/unit/test_runtime_dependency_contract.py`
- app-profile boot smoke on 2.15.0: `boot_profile=default-apps`, `app_plugins_loaded=True`,
  `app_plugin_count=16`
...
- the declared dependency in `pyproject.toml` is `aindy-runtime>=2.15.0,<3.0`
