...
- installed runtime version: `2.20.0` (checked by **import and path** —
  ...
  `RUNTIME_2_12_0_UPGRADE.md` §1; re-checked clean on every adoption since, latest `RUNTIME_2_20_0_UPGRADE.md` §1.
...
- apps repo dependency: range `aindy-runtime>=2.20.0,<3.0` in `pyproject.toml`, build pin
  `aindy-runtime==2.20.0` in `constraints.txt`, asserted equal by
  `tests/unit/test_runtime_dependency_contract.py`
- app-profile boot smoke on 2.20.0: `boot_profile=default-apps`, `app_plugins_loaded=True`,
  `app_plugin_count=16`
...
- the declared dependency in `pyproject.toml` is `aindy-runtime>=2.20.0,<3.0`
